...
<div class="row-fluid">
	<aui:fieldset cssClass="span6">
		...
		<liferay-ui:error exception="<%=UserFieldException.class%>">
			...
			<liferay-ui:message arguments="<%=sb.toString()%>"
		...
		<liferay-ui:error exception="<%=UserScreenNameException.class%>"
			focusField="screenName" message="please-enter-a-valid-screen-name" />

		<c:if
			test="<%=!PrefsPropsUtil.getBoolean(company.getCompanyId(),
							PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || (selUser != null)%>">
			<c:choose>
				<c:when
					test='<%=PrefsPropsUtil.getBoolean(company.getCompanyId(),
									PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE)
									|| !UsersAdminUtil.hasUpdateFieldPermission(permissionChecker, user, selUser,
											"screenName")%>'>
		...
		<c:choose>
			<c:when
				test='<%=!UsersAdminUtil.hasUpdateFieldPermission(permissionChecker, user, selUser,
								"emailAddress")%>'>
			...
			<c:otherwise>
				...
				<aui:input bean="<%=displayEmailAddressUser%>"
		...
		<%@ include
			file="/html/portlet/users_admin/user/details_user_name.jspf"%>
	</aui:fieldset>
			...
			<c:if test="<%=selUser != null%>">
				<c:choose>
					<c:when
						test='<%=UsersAdminUtil.hasUpdateFieldPermission(permissionChecker, user, selUser,
									"portrait")%>'>
						<portlet:renderURL var="editUserPortraitURL"
							windowState="<%=LiferayWindowState.POP_UP.toString()%>">
							<portlet:param name="struts_action"
								value="/users_admin/edit_user_portrait" />
							<portlet:param name="redirect" value="<%=currentURL%>" />
							<portlet:param name="p_u_i_d"
								value="<%=String.valueOf(selUser.getUserId())%>" />
							<portlet:param name="portrait_id"
								value="<%=String.valueOf(selUser.getPortraitId())%>" />
						</portlet:renderURL>
					...
					<c:otherwise>
						<img alt="<liferay-ui:message key="portrait" />"
		...
		<c:if test="<%=selUser != null%>">
			...
			<aui:field-wrapper name="userId">
				<liferay-ui:input-resource
					url="<%=String.valueOf(selUser.getUserId())%>" />
