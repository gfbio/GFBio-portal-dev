...
public class CustomUpdatePasswordAction extends UserLocalServiceWrapper {
	...
	/*
	 * @Override public User updateUser(long userId, String oldPassword, String
	 * newPassword1, String newPassword2, boolean passwordReset, String
	 * reminderQueryQuestion, String reminderQueryAnswer, String screenName,
	 * String emailAddress, long facebookId, String openId, String languageId,
	 * String timeZoneId, String greeting, String comments, String firstName,
	 * String middleName, String lastName, int prefixId, int suffixId, boolean
	 * male, int birthdayMonth, int birthdayDay, int birthdayYear, String smsSn,
	 * String aimSn, String facebookSn, String icqSn, String jabberSn, String
	 * msnSn, String mySpaceSn, String skypeSn, String twitterSn, String ymSn,
	 * String jobTitle, long[] groupIds, long[] organizationIds, long[] roleIds,
	 * List<UserGroupRole> userGroupRoles, long[] userGroupIds, ServiceContext
	 * serviceContext) throws PortalException, SystemException { // called when
	 * login, change password, reset password log.info(":: Hook on update user."
	 * ); if (groupIds != null)log.info(":: groupIds:"
	 * +Arrays.toString(groupIds)); if (organizationIds != null)log.info(
	 * ":: organizationIds:"+Arrays.toString(organizationIds)); if (roleIds !=
	 * null)log.info(":: roleIds:"+Arrays.toString(roleIds)); if (userGroupRoles
	 * != null) log.info(":: userGroupRoles:"+ userGroupRoles.toString()); if
	 * (userGroupIds !=null)log.info(":: userGroupIds:"
	 * +Arrays.toString(userGroupIds));
	 * 
	 * 
	 * User user = super.getUser(userId); if (emailAddress != null) log.info(
	 * ":: emailAddress:"+emailAddress); try { // if (!isReservedEmail(user)) {
	 * // If the email is used by the backend (e.g. Portal mail // settings),
	 * the password should not be updated // } log.info(":: updateUser "
	 * +userId); log.info(":: updateUser "+newPassword1); user =
	 * super.updateUser(userId, oldPassword, newPassword1, newPassword2,
	 ...
	 * serviceContext); if (newPassword1.trim() != "" && newPassword1 != null) {
	 * updatePassword(userId, newPassword1,newPassword2, false); log.info(
	 * ":: updatePassword"); } } catch (Exception e) { e.printStackTrace();
	 * log.error(e.toString()); return null; } return user; }
	 */
	...
	public void sendEmailAddressVerification(
		...
		log.info(":: Hook on sendEmailAddressVerification.");
		try {
			
			String password = (String)serviceContext.getAttribute(
						"passwordUnencrypted");
			
			/*boolean isLDAPUserAdded = LDAPUserAccount.LDAPaddUser(user, password);
			
			/*if (isLDAPUserAdded) { // create JIRA account
				log.info(":: LDAP user successfully added");
				URLBasicAuth.urlAuth(emailAddress, password);
			}*/
	...
	public User updatePassword(long userId, String password1, String password2, boolean passwordReset)
		...
		log.info(":: Hook on updatePassword.");
	...
	public User updateEmailAddress(long userId, String password, String emailAddress1, String emailAddress2)
			throws PortalException, SystemException {
		log.info(":: Hook on updateEmailAddress.");
		User user = super.updateEmailAddress(userId, password, emailAddress1, emailAddress2);
		try {
			boolean isUserAdded = LDAPUserAccount.LDAPaddUser(user, password);
			if (isUserAdded) {
				log.info(":: LDAP user successfully added");
				URLBasicAuth.urlAuth(user.getEmailAddress(), password);
			}
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e.toString());
